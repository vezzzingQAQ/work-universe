...
start = (dom, f, rate = 1) => {
    ...
    if (dom.offsetWidth > 500) {
        canvas.ioarea.style.width = "min-content";
...
ellipse = (x, y, rx, ry, rotation = 0) => {
    context.ellipse(x - rx / 2, y - ry / 2, rx, ry, rotation, 0, Math.PI * 2);
}
ellipseCenter = (x, y, rx, ry, rotation = 0) => {
...
class RGBColor {
    ...
    darker(rate) {
        this.r *= (map(rate, 0, 100, 255, 0) / 255);
        this.g *= (map(rate, 0, 100, 255, 0) / 255);
        this.b *= (map(rate, 0, 100, 255, 0) / 255);
        return (this);
    }
}
